...
2、定投当日查看要定投的指数基金的估值,根据估值判断这个月投入的资金。比如,如果估值为高投4000,估值正常投8000,估值为低投12000。(实际每月投入金额和估值的关系可能复杂点,估值可以参考天天基金或支付宝)
...
解释:指数基金定投份额确认日是当月的最低点的概率很低,所以后面那小部分的钱大概率是能全进去的(补仓次数设置越少,每个月预计投入额完全投完概率越高)
...
## 项目功能
每天(交易日)14.55查询要定投的指数基金净值,和当月定投日确认份额时的净值比较,如果比他低了自己的预设值(比如可以设置为低于定投当日确认份额时净值的百分之99.8%),就给指定手机号发短信，提示给这个基金补仓。当本月补仓次数已满(可以自己设置,比如设置每月补仓次数为3次,分3次把当月预计投入剩余的钱投完)，则这个月后续不会再比较净值并发短信。
